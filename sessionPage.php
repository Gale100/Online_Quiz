...
if(isset($_POST['startquiz']))
{
	...
	if((is_numeric($_POST['studentID'])) && (strlen($_POST['studentID']) == 9)) {
		//student ID is valid
		if(ctype_alpha(trim($_POST['firstName']))){ 
			//first name valid
				 if(ctype_alpha($_POST['lastName'])){
					//lastname valid
		 			 if((is_numeric($_POST['age'])) &&  (($_POST['age']) < 100 )){ 
						//age valid
							include('inc_databaseConnect.php'); 
						}else{
							 echo '<script >
								window.alert("age must be valid integer. Please submit the form again.");</script>';
			...
			echo '<script >
			window.alert("please enter a valid 9-digit Integer  in Id field and submit the form again.");</script>';
			include('homePage.php');
